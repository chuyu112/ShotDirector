...
项目另存两份独立源码附件，入口见 [模块附件目录](attachments/README.md)：

- [心流 V1.3](attachments/xinliu/README.md)：小说文本、分镜草稿、视觉规则、续写/缩写与资产描述，附原包和审阅资料。
- [Kunpeng Director](attachments/kunpeng/README.md)：3D 白模、人物动作、运镜与时间轴，附固定提交 `1c629c9` 的完整源码和 MIT 许可证。

2026-09-09 确定采用方式：提取两者有价值的设计，直接融入漫镜现有分镜、资产、提示词与白模流程，不单独开发插件或另建工作台。具体可复用点、现有实现和待增强能力见 [优点采用说明](docs/漫镜-参考项目优点采用.md)。附件保留用于溯源；来源和完整性记录见 [附件清单](attachments/manifest.json)。
...
该命令会先执行 TypeScript 类型检查，再构建前端并运行结构、桥接与工作流测试；类型错误会直接阻止后续步骤。单独检查类型可运行 `npm run typecheck`。
