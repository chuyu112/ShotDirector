...
test("strict review keeps waiting through transient disconnects and result-file races", () => {
  ...
  assert.deepEqual(promptReviewRecoveryDecision({ httpStatus: 202, resultStatus: "queued", startedAt, now }), { action: "waiting" });
...
test("strict review only surfaces a confirmed failure or an expired recovery window", () => {
  ...
  assert.deepEqual(promptReviewRecoveryDecision({ httpStatus: 200, resultStatus: "interrupted", now }), { action: "failed", reason: "terminal" });
  assert.deepEqual(promptReviewRecoveryDecision({ httpStatus: 200, resultStatus: "aborted", now }), { action: "failed", reason: "terminal" });
