...
- 生产冒烟环境文件改为 `mktemp` 随机路径、权限 `0600`，在同一次远程命令内使用并通过 trap 清理；假密钥回归覆盖准备失败和信号终止，发布后的真实冒烟也已通过。
- Shot 卡片右侧元数据允许列内换行，保留时长输入控件；本地浏览器覆盖桌面、窄屏、手机与横屏。
- 本地 `npm test`（类型检查、构建、332 项测试）、`npm run test:harness`（34 项）、lint 均通过。Provider 使用测试替身，另运行真实本地 Worker 集成回归。
- 用户随后明确授权推送与部署：修复提交 `720832f` 已推送当前分支，于 2026-09-20 06:43:40（Asia/Shanghai）发布到 `43.173.105.8` 的 Docker Compose，版本 `20260919T223930Z-720832f`。公网资源、数据完整性与隔离容器中的真实 Creator / Reviewer 冒烟通过，完整证据与回滚见 `docs/2026-09-20-审查修复部署.md`。
