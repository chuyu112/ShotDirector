...
画格若含 humanConfirmation，则 sourceObservation 与 sourceText 来自人工对原裁图的核对；machineEvidence 仅为保留溯源的旧模型说法，不能覆盖人工结论。textStatus=unreadable 时保留无法辨读，不补写台词；发现新的证据冲突只报告并等待再次人工核对，不自动改写事实。
